...
import { mockVideos, Video, deleteVideo } from "@/data/videos"; // Import deleteVideo
...
const VideoListingPage: React.FC = () => {
  ...
  useEffect(() => {
    setCurrentVideos([...mockVideos]); // Update local state when global mock data changes
  }, [mockVideos]);

  const handleDeleteVideo = (videoId: string) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus video ini?")) {
      deleteVideo(videoId);
      setCurrentVideos([...mockVideos]); // Refresh local state
    ...
    <>
      ...
      <div className="container mx-auto p-4 md:p-8">
        <div className="flex justify-between items-center mb-10">
          ...
          {userRole === "seller" && (
            <Button onClick={() => navigate("/seller/videos/new")} className="bg-soft-pink hover:bg-rose-600 text-white font-poppins">
        ...
        {currentVideos.length === 0 ? (
          <div className="text-center p-10 bg-white dark:bg-gray-800 rounded-lg shadow-md">
            ...
            {userRole === "seller" && (
              <Button onClick={() => navigate("/seller/videos/new")} className="px-8 py-3 text-lg bg-soft-pink hover:bg-rose-600 text-white font-poppins">
                Tambah Video Pertama Anda
              </Button>
            )}
            <Link to="/">
              <Button variant="outline" className="mt-4 px-8 py-3 text-lg border-soft-pink text-soft-pink hover:bg-soft-pink hover:text-white font-poppins">
                Kembali ke Beranda
              </Button>
            </Link>
          </div>
              ...
              <Card key={video.id} className="overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 border-2 border-beige dark:border-gray-700 bg-white dark:bg-gray-800">
                ...
                {userRole === "seller" && (
                  <CardFooter className="p-4 pt-0 flex justify-end space-x-2">
                    <Button variant="outline" size="sm" onClick={() => navigate(`/seller/videos/edit/${video.id}`)} className="border-gold-rose text-gold-rose hover:bg-gold-rose hover:text-white font-poppins dark:border-gold-rose dark:text-gold-rose dark:hover:bg-gold-rose dark:hover:text-white">
                      <Edit className="h-4 w-4 mr-1" /> Edit
                    </Button>
                    <Button variant="destructive" size="sm" onClick={() => handleDeleteVideo(video.id)} className="font-poppins">
